...
public class PickListRfAction implements Serializable {
	...
	@In(value = "#{tableRfVcvMatrixInit.allRiskFactors}")
	private List<MarketVariable> allRiskFactors;

	@In(value = "#{tableRfVcvMatrixInit.pickRiskFactors}")
	...
	public void loadPickList() {
		...
		// riskFactors =
		// entityManager.createQuery(NamedQuery.RiskFactors.getQuery()).getResultList();
	...
	public void afterPick() {
		// log.info("afterPick Id1 :#0,#1",
		// Conversation.instance().getId(),Conversation.instance().isLongRunning());
